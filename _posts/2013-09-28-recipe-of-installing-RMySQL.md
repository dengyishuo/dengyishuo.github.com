...
处理金融市场数据的时候会用到 `MySQL`,`R` 提供了一个 `RMySQL` 包作为自身跟 `MySQL` 的接口。这个包本身很容易用，唯一不足的一点是在 `Windows` 下面安装 `RMySQL` 略费周折，基本跟驯服一头猛兽差不多,当然是一头不怎么听话的猛兽.安装 `RMySQL` 时,除了按部就班的安装`MySQL`, `R` ,
`RTools`,`RMySQL`等之外还要折腾各种`path`变量以及把某些文件夹在各个路径里来回移动.这篇文章就说一下自己在安装`RMySQL`过程中总结出来的较优的安装步骤.
...
具体做法是找到我的电脑(指图标),右键,系统属性,打开"高级"选项卡,下面可以瞅见"环境变量",点进去,上面部分为"用户变量",下面部分是"系统变量",在变量列，找到一个叫 Path 的东西,选中,点编辑,把 MySQL 的安装路径复制进去即可;
