...
可以将ts，irts，its等对象转化为zoo对象，如果索引正确，还可以将zoo对象转化回去。zoo对象和zooreg对象之间的转化更方便。除此之外，zoo对象还可以转化为数据框。
...
na.omit，na.contiguous，na.approx，na.locf，na.omit和na.spline都能来处理缺失值。na.contiguous函数是返回对象中没有不包含缺失值的最长片段，na.approx函数是对缺失值继续线性差值，na.lcof是用前值替代法进行差值。
