...
ggplot() +
  ...
  geom_text_repel(
    data = attractions,
    aes(x = lon, y = lat, label = name),
    nudge_y = 0.005,  # 微调标签位置
    size = 2, 
    color = "black",
    segment.size = 0.3  # 添加连接点与标签的细线
  ) +
  
  # 缩小地图范围至平城区核心（景点密集区）
  xlim(113.27, 113.31) +  # 经度范围缩小
  ylim(40.09, 40.11)      # 纬度范围缩小
  theme_void() +
  ggtitle("大同平城区核心景点分布") +
